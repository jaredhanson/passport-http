...
const nonce_charlen = nonce_chars.length
...
const nonce = len => {
  ...
  for (var i = 0; i < len; ++i) {
    buf.push(nonce_chars[Math.random() * nonce_charlen | 0])
  }
 ...
 * @api private
 */
const md5 = (str, encoding = 'hex') => crypto
  .createHash('md5')
  .update(str)
  .digest(encoding)
 ...
 *   - `qop`        list of quality of protection values support by the server
 *                  (auth | auth-int) (recommended: auth)
 *
 * `validate` params:
 *   - `nonce`   unique string value specified by the server
 *   - `cnonce`  opaque string value provided by the client
 *   - `nc`      count of the number of requests (including the current request)
 *               that the client has sent with the nonce value
 *   - `opaque`  string of data, specified by the server, which should be
 *               returned by the client in subsequent requests
 *
 * Examples:
 *
...
class DigestStrategy {
  ...
  constructor(options, secret, validate) {
 ...
 */
util.inherits(DigestStrategy, Strategy)
