 ...
 * as a `verify` callback which accepts `username` and `options`, and then
 ...
 * set.
 *
 * It should be noted that when `verify` is called, the request will already
 * have been successfully authenticated based on username and password
 * credentials.  The `verify` method can further inspect nonce-related
 ...
 *   - `algorithm`  algorithm used to produce the digest (MD5 | MD5-sess | ha1)
 ...
 * @api public
 */
function DigestStrategy(options, secret, verify) {
  ...
  if (!verify) throw new Error('HTTP Digest authentication strategy requires a verify function');
