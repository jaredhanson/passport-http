...
DigestStrategy.prototype.authenticate = function(req) {
  ...
  if (req.url !== creds.uri) {
    // FIXME: This should be returned to the client as a 400 Bad Request.
    return this.error(new Error('bad request'));
  }
  ...
  this.secret(creds.username, function(err, password) {
    ...
    if (!creds.algorithm || creds.algorithm === 'MD5') {
      ...
      return self.error(new Error('unknown algorithm'));
    }
    ...
    if (!creds.qop || creds.qop === 'auth') {
      ...
      return self.error(new Error('unknown quality of protection'));
    }
